...
void* organizar(void* x) {
    ajuda* y = (ajuda*) x;
    printf("Thread %d processando tarefa %d\n", y->id_thread, y->id_tarefa);
    bubble_sort((int *)y->vetor, y->tamanho);
    return NULL;
}
...
int sort_paralelo(unsigned int *vetor, unsigned int tam, unsigned int ntasks, unsigned int nthreads) {
    ...
    for (int i = 0; i < ntasks; i++) {
        ...
        ajudas[i].vetor = &vetor[offset];
        ajudas[i].tamanho = tamanho;
        ajudas[i].id_tarefa = i;
        ajudas[i].id_thread = i % nthreads;  // Apenas uma simulação de ID de thread
        pthread_create(&threads[i], NULL, organizar, &ajudas[i]);
        offset += tamanho;
    }
